...
<template>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'operation'">
          <a class="color-primary ml-5 mr-5 whitespace-nowrap" @click="onEdit(1, record)"
            >关联api</a
          >
          <a-popconfirm
          ...
          >
            <a class="color-primary ml-5 mr-5 whitespace-nowrap">禁用</a>
          </a-popconfirm>
          <a
            v-if="record.status !== 1"
            class="color-primary ml-5 mr-5 whitespace-nowrap"
            @click="onEdit(3, record)"
            >启用</a
          >
          <a class="color-primary ml-5 mr-5 whitespace-nowrap" @click="onEdit(4, record)"
            >添加下级菜单</a
          >
          <a class="color-primary ml-5 mr-5 whitespace-nowrap" @click="onEdit(5, record)">编辑</a>
  ...
  <edit-menu ref="editRef" :menuList="dataSource" @change="(val) => handleChangeEdit(1, val)" />
 ...
 */
const onChangeStatus = async ({ status, key }) => {
  try {
    ...
    // }
    dataSource.value = changeTreeData(dataSource.value, 'key', key, 'status', status === 1 ? 0 : 1)
 ...
 * @param {Number} type 1:菜单，2：api
 * @param {Object} row 当前数据
 */
const handleChangeEdit = (type, val) => {
  const tmp = {
    1: () => {
      console.log('处理菜单更新', val)
    },
    2: () => {
      console.log('处理api更新', val)
    }
  }
  tmp[type]()
}
 ...
 * @description 菜单操作
 * @param {Object} row 当前数据
 * @param {Number} type 0:新增,1:关联api,2:禁用,3:启用,4:添加下级菜单,4:编辑,6:删除
 */
...
const onEdit = useDebounceFn(
  async (type, row) => {
    ...
    const tmps = {
      ...
      4: () => editRef.value.open(type, { ...row, menuType: 2 }),
      5: () => editRef.value.open(type, { ...row, menuType: 2 }),
